...
export default function AdminProducts() {
  ...
  const [deleteId, setDeleteId] = useState(null); // product id awaiting confirm
  ...
  async function handleSave() {
    ...
    try {
      ...
      const url = modal === 'edit' ? `${API}/api/products/${editing.id}` : `${API}/api/products`;
    ...
    <AdminLayout>
          ...
          <table className="w-full text-sm">
            <thead className="bg-stone-50 border-b border-stone-200">
              <tr>
                ...
                <th className="text-right px-4 py-3 font-medium text-stone-600">الاسم</th>
                <th className="text-right px-4 py-3 font-medium text-stone-600">الفئة</th>
                <th className="text-right px-4 py-3 font-medium text-stone-600">السعر</th>
                <th className="text-right px-4 py-3 font-medium text-stone-600">المخزون</th>
                <th className="text-right px-4 py-3 font-medium text-stone-600">الحالة</th>
        ...
        <Modal title={modal === 'add' ? 'إضافة منتج جديد' : 'تعديل المنتج'} onClose={closeModal}>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                className="border border-stone-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400"
              >
          ...
          <div className="flex gap-2 justify-end mt-5">
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex items-center gap-1.5 px-4 py-2 text-sm rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-60 transition-colors"
            >
      ...
      {deleteId && (
        <Modal title="تأكيد الحذف" onClose={() => setDeleteId(null)}>
          <p className="text-stone-600 text-sm mb-5">هل أنت متأكد من حذف هذا المنتج؟ لا يمكن التراجع.</p>
